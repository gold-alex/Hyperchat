...
export class EnvelopeReceiverFilter {
  ...
  async evaluateMessage(message) {
    ...
    const fakeEnvelope = {
      ...
      messageId: message.messageId || deriveMessageId(message),
  ...
  async _evaluate(envelope, metadata) {
    if (shouldVerifyEnvelopeSignature(envelope, metadata)) {
      const signatureValid = await verifyEnvelopeSignature(envelope, metadata);
      if (!signatureValid) {
...
async function verifyEnvelopeSignature(envelope, metadata) {
  try {
    ...
    if (normalizeHex(etc.bytesToHex(hash)) !== normalizeHex(envelope.messageId)) {
      return false;
    }
    return verify(String(envelope.signature), hash, String(envelope.sessionPubKey));
  } catch {
    return false;
  }
}
