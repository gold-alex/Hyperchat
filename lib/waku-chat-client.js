...
class WakuChatClient {
  constructor(config = {}) {
    ...
    const wakuNodeConfig = parseEndpointConfig(config.wakuNodeURI ?? 'localhost', 'localhost');
    this.wakuNodeURI = wakuNodeConfig.host || 'localhost';
    this.wakuNodeEndpointScheme = wakuNodeConfig.scheme;
    ...
    this.wakuBootstrapPeersRaw = config.wakuBootstrapPeers ?? config.wakuBootstrapPeerList ?? '';
    this.wakuBootstrapPeerMaxEntries = typeof config.wakuBootstrapPeerMaxEntries === 'number'
      ? config.wakuBootstrapPeerMaxEntries
      : undefined;
    ...
    this.gatewayUrl = (config.gatewayUrl || config.gatewayURL || '').trim();
    ...
    this.gatewayDomain = resolveGatewayDomain({
      gatewayUrl: this.gatewayUrl,
      configuredGatewayDomain: config.gatewayDomain,
      allowOverride:
        config.allowGatewayDomainOverride === true
    ...
    this.useLegacyProto = config.useLegacyProto === true;
    this.session = null;
    this.siweSigner = config.signMessage || null;
    this._sessionStoragePendingClear = null;
    this.activeBootstrapPeer = null;

    if (this.useLegacyProto) {
      console.warn('WakuChatClient running in explicit legacy protobuf mode (auth-lite gateway send disabled).');
    } else if (!this.gatewayUrl) {
      console.warn('WakuChatClient strict auth-lite mode active without gateway URL; send/session calls will fail closed.');
    }
  ...
  async initialize() {
    try {
        ...
        const bootstrapPeers = this._resolveBootstrapPeers();
        console.log('Configured Waku bootstrap peers:', bootstrapPeers.map((peer) => peer.multiaddr));
        ...
        for (let index = 0; index < bootstrapPeers.length; index += 1) {
          ...
          const isLocal = peer.isLocalHost === true;
          const attemptLabel = `${index + 1}/${bootstrapPeers.length}`;
          const libp2pConfig = isLocal
            ? { filterMultiaddrs: false, hideWebSocketInfo: true }
            : { hideWebSocketInfo: true };

          console.log(`Waku bootstrap attempt ${attemptLabel}: ${peer.multiaddr}`);
          let candidateNode = null;

          try {
            candidateNode = await createLightNode({
            ...
            await candidateNode.start();
            await waitForRemotePeer(candidateNode, [Protocols.Filter, Protocols.LightPush], 15000);

            this.waku = candidateNode;
            this.activeBootstrapPeer = peer;
            if (index > 0) {
              console.warn(`Waku bootstrap failover selected peer ${attemptLabel}: ${peer.multiaddr}`);
            }
            break;
          } catch (attemptError) {
            const reason = getErrorMessage(attemptError);
            attemptFailures.push({ peer: peer.multiaddr, reason });
            console.warn(`Waku bootstrap attempt ${attemptLabel} failed: ${peer.multiaddr} (${reason})`);
            if (candidateNode) {
              try {
                await candidateNode.stop();
              } catch {
                // Ignore node stop errors while continuing deterministic failover.
              }
            }
          }
        }
        ...
        if (!this.waku) {
          const detail = attemptFailures.map((item) => `- ${item.peer}: ${item.reason}`).join('\n');
          throw createClientConfigError(
            'WAKU_BOOTSTRAP_UNAVAILABLE',
            `Unable to connect to any configured Waku bootstrap peer.\n${detail}`,
          );
        }
  ...
  async _sendMessageViaGateway(content) {
    ...
    this._assertGatewayConfigured('message publish');
  ...
  async _ensureSession() {
    this._assertGatewayConfigured('session establishment');
    const contentTopic = this.getContentTopic();
    const pubsubTopic = this.getPubSubTopic();
  ...
  async _gatewayRequest(path, body) {
    this._assertGatewayConfigured(`gateway request ${path}`);
  ...
  _assertGatewayConfigured(operation) {
    if (this.useLegacyProto) {
      return;
    }
    if (!this.gatewayUrl) {
      throw createGatewayError(
        ...
        `Strict auth-lite mode cannot continue: gateway URL is missing for ${operation}. Set VITE_LIGHTPUSH_GATEWAY_URL or explicitly opt into legacy mode (useLegacyProto=true).`,
        400,
      );
    }
    this._assertGatewayTransportPolicy(operation);
  }

  _assertGatewayTransportPolicy(operation) {
    const parsed = parseUrlOrNull(this.gatewayUrl);
    if (!parsed) {
      throw createGatewayError(
        'GATEWAY_CONFIG_INVALID',
        `Strict auth-lite mode cannot continue: gateway URL "${this.gatewayUrl}" is invalid for ${operation}. Configure a valid https URL (or http on localhost/loopback only).`,
        400,
      );
    }
    ...
    if (protocol !== 'http' && protocol !== 'https') {
      throw createGatewayError(
        'GATEWAY_CONFIG_INVALID_SCHEME',
        `Strict auth-lite mode cannot continue: gateway URL "${this.gatewayUrl}" uses unsupported scheme "${protocol}" for ${operation}. Use https (or http on localhost/loopback only).`,
        400,
      );
    }
    if (protocol === 'http' && !isLoopbackHost(host)) {
      throw createGatewayError(
        'GATEWAY_INSECURE_TRANSPORT',
        `Strict auth-lite mode cannot continue: insecure gateway transport is not allowed for non-local host "${host}" during ${operation}. Use https:// for remote gateways.`,
        400,
      );
    }
  }

  _resolveBootstrapPeers() {
    const contextLabel = 'Waku bootstrap peers';
    const rawList = this.wakuBootstrapPeersRaw;
    const maxEntries = this.wakuBootstrapPeerMaxEntries;
    try {
      const hasListInput =
        (typeof rawList === 'string' && rawList.trim().length > 0)
        || (Array.isArray(rawList) && rawList.length > 0);
      if (hasListInput) {
        const peers = parseBootstrapPeerList(rawList, { contextLabel, maxEntries });
        if (peers.length === 0) {
          throw createClientConfigError(
            'WAKU_BOOTSTRAP_EMPTY',
            `${contextLabel}: no usable entries after parsing.`,
          );
        }
        return peers;
      }

      const legacyPeer = buildLegacyBootstrapPeerFromHost({
        contextLabel: `${contextLabel} (legacy fallback)`,
        host: this.wakuNodeURI,
        port: this.wakuNodePort,
        peerId: this.wakuNodePeerId,
        schemeHint: this.wakuNodeEndpointScheme,
      });
      return [legacyPeer];
    } catch (error) {
      if (error?.code) {
        throw createClientConfigError(error.code, error.message);
      }
      throw error;
    }
  }

  _resolveWakuTransport() {
    const host = String(this.wakuNodeURI || '').trim().toLowerCase();
    const scheme = String(this.wakuNodeEndpointScheme || '').trim().toLowerCase();
    const isLocal = isLocalDevelopmentHost(host);

    if (scheme && scheme !== 'ws' && scheme !== 'wss') {
      throw createClientConfigError(
        'WAKU_CONFIG_INVALID_SCHEME',
        `Invalid Waku node endpoint scheme "${scheme}". Provide a host (for inferred transport) or a ws/wss URL.`,
      );
    }
    if (scheme === 'ws' && !isLocal) {
      throw createClientConfigError(
        'WAKU_INSECURE_TRANSPORT',
        `Insecure Waku transport "ws" is not allowed for non-local host "${host}". Use wss or a host-only endpoint for remote nodes.`,
      );
    }

    return {
      isLocal,
      wsProto: isLocal ? 'ws' : 'wss',
    };
  }
...
function resolveGatewayDomain(params) {
  const derivedDomain = deriveGatewayDomain(params.gatewayUrl);
  const configuredDomain = normalizeGatewayDomain(params.configuredGatewayDomain);
  if (!configuredDomain) {
    return derivedDomain;
  }
  if (!params.gatewayUrl || configuredDomain.toLowerCase() === derivedDomain.toLowerCase()) {
    return configuredDomain;
  }
  if (params.allowOverride) {
    console.warn(
      `WakuChatClient using explicit gatewayDomain override "${configuredDomain}" for gateway "${params.gatewayUrl}".`,
    );
    return configuredDomain;
  }
  throw createClientConfigError(
    'GATEWAY_DOMAIN_OVERRIDE_DISALLOWED',
    `Strict auth-lite mode requires SIWE domain to match selected gateway hostname "${derivedDomain}" but got override "${configuredDomain}". Use override only in development/local contexts.`,
  );
}
...
function parseEndpointConfig(rawValue, defaultHost = '') {
  const trimmed = String(rawValue ?? '').trim();
  if (!trimmed) {
    return { host: defaultHost, scheme: '' };
  }
...
function isDevelopmentRuntime() {
  const nodeEnv = String(globalThis?.process?.env?.NODE_ENV || '')
    .trim()
    .toLowerCase();
...
function createClientConfigError(code, message) {
