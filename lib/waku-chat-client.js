...
class WakuChatClient {
  constructor(config = {}) {
    ...
    const env = (typeof import.meta !== 'undefined' && import.meta && import.meta.env)
      ? import.meta.env
      : {};
    this.wakuNodeURI = env.VITE_WAKU_NODE_URI ?? config.wakuNodeURI ?? 'localhost';
    // Ensure port is a number
    const portFromEnv = env.VITE_WAKU_NODE_PORT;
    const normalizedPort = typeof portFromEnv === 'string' ? parseInt(portFromEnv, 10) : portFromEnv;
    this.wakuNodePort = normalizedPort ?? config.wakuNodePort ?? 443;
    this.wakuNodePeerId = env.VITE_WAKU_NODE_PEER_ID ?? config.wakuNodePeerId ?? 'PEER_ID';
